...
var OPACITY = {
  ...
  TYPES = ["\\","renewals", "children", "childcare", "work-and-hours", "dropoff", "others"],
...
var formatNumber = function (d) {
  var numberFormat = d3.format("d"); // zero decimal places
  return numberFormat(d);
},

formatFlow = function (d) {
  var flowFormat = d3.format("d"); // zero decimal places with sign
  return flowFormat(Math.abs(d)) + " visits";
},
...
d3.json("sankey/sankey.json", function(error, graph) {
  biHiSankey
    .nodes(graph.nodes)
    .links(graph.links)
    .initializeNodes(function (node) {
